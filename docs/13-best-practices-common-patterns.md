...
public class ResourceCleanupPatterns {
    ...
    public void nestedResourceExample(IgniteClient client) {
        try (ResultSet<SqlRow> resultSet = client.sql().execute(null, "SELECT * FROM Artist")) {
...
public class TransactionBoundaryPatterns {
    ...
    public void explicitTransactionBoundaries(IgniteClient client) {
        try (Transaction tx = client.transactions().begin()) {
            ...
            Album album = new Album(1L, "Abbey Road", 1L);
    ...
    public void functionalTransactionAPI(IgniteClient client) {
        try {
            client.transactions().runInTransaction(tx -> {
                ...
                Artist artist = new Artist(2L, "Led Zeppelin");
                artistView.upsert(tx, artist);
                
                Album album = new Album(2L, "Led Zeppelin IV", 2L);
                albumView.upsert(tx, album);
    ...
    public CompletableFuture<Void> asyncTransactionPattern(IgniteClient client) {
        return client.transactions().runInTransactionAsync(tx -> {
            ...
            Artist artist = new Artist(3L, "Pink Floyd");
...
public class GracefulDegradationPatterns {
    ...
    public class ArtistServiceWithFallback {
        private final IgniteClient igniteClient;
        private final Map<Long, Artist> localCache = new ConcurrentHashMap<>();
        private final AtomicBoolean igniteAvailable = new AtomicBoolean(true);
        
        public ArtistServiceWithFallback(IgniteClient igniteClient) {
        ...
        public Artist getArtist(Long id) {
            if (igniteAvailable.get()) {
                try {
                    Artist artist = getFromIgnite(id);
                    localCache.put(id, artist); // Update cache on success
                    return artist;
            ...
            Artist cached = localCache.get(id);
            if (cached != null) {
                System.out.println("Serving from fallback cache for artist: " + id);
                return cached;
            }
            
            throw new RuntimeException("Artist not found and Ignite unavailable");
        }
        
        private Artist getFromIgnite(Long id) {
    ...
    public class CircuitBreakerWithFallback {
        ...
        public List<Artist> getArtistsByGenre(String genre) {
            ...
            try {
                List<Artist> artists = getArtistsFromIgnite(genre);
                onSuccess();
                return artists;
                
            } catch (Exception e) {
                onFailure();
                return getFallbackArtists(genre);
            }
        }
        
        private List<Artist> getArtistsFromIgnite(String genre) {
            try (ResultSet<Artist> resultSet = igniteClient.sql().execute(
                    null, Mapper.of(Artist.class), 
                    "SELECT a.* FROM Artist a JOIN Album al ON a.ArtistId = al.ArtistId JOIN Track t ON al.AlbumId = t.AlbumId JOIN Genre g ON t.GenreId = g.GenreId WHERE g.Name = ?", genre)) {
                
                List<Artist> artists = new ArrayList<>();
                while (resultSet.hasNext()) {
                    artists.add(resultSet.next());
                }
                return artists;
            }
        }
        
        private List<Artist> getFallbackArtists(String genre) {
            System.out.println("Using fallback data for genre: " + genre);
            // Return cached or default data
            return Arrays.asList(
                new Artist(0L, "Fallback Artist")
            );
        }
...
public class PerformanceGuidelines {
    ...
    public class OptimalQueryPatterns {
        ...
        public List<Artist> getArtistsByNamePattern(String pattern) {
            // Reuse prepared statement
            try (ResultSet<Artist> resultSet = client.sql().execute(
                    null, customerByCountryStmt, Mapper.of(Artist.class), pattern)) {
    ...
    public void efficientBatchOperations(IgniteClient client, List<Artist> artists) {
    ...
    public CompletableFuture<Map<String, List<Track>>> getTracksByGenresAsync(IgniteClient client, 
                                                                                        List<String> genres) {
        List<CompletableFuture<Map.Entry<String, List<Track>>>> futures = genres.stream()
            .map(genre -> 
                client.sql().executeAsync(null, Mapper.of(Track.class), 
    ...
    public void badQueryPattern(IgniteClient client, List<Long> artistIds) {
        RecordView<Artist> view = client.tables().table("Artist").recordView(Artist.class);
        
        for (Long id : artistIds) {
            Artist key = new Artist();
            key.setArtistId(id);
            Artist artist = view.get(null, key); // Individual query for each ID
            processArtist(artist);
        }
    }
    
    // ✅ GOOD: Single query to get all data
    public void goodQueryPattern(IgniteClient client, List<Long> artistIds) {
        ...
        try (ResultSet<Artist> resultSet = client.sql().execute(
                null, Mapper.of(Artist.class), sql, artistIds.toArray())) {
            
            while (resultSet.hasNext()) {
                Artist artist = resultSet.next();
                processArtist(artist);
            }
        }
    }
    ...
    public class ConnectionReusePattern {
        ...
        private List<Artist> getArtists() {
            try (ResultSet<Artist> resultSet = client.sql().execute(
                    null, Mapper.of(Artist.class), "SELECT * FROM Artist LIMIT 100")) {
        ...
        private List<Album> getAlbums() {
            try (ResultSet<Album> resultSet = client.sql().execute(
                    null, Mapper.of(Album.class), "SELECT * FROM Album LIMIT 100")) {
                
                List<Album> albums = new ArrayList<>();
                while (resultSet.hasNext()) {
                    albums.add(resultSet.next());
                }
                return albums;
            }
        }
...
public class MemoryManagementBestPractices {
    ...
    public void streamLargeResultSets(IgniteClient client) {
        try (ResultSet<Track> resultSet = client.sql().execute(
            ...
            int batchSize = 0;
            while (resultSet.hasNext()) {
                Track track = resultSet.next();
                processTrack(track);
                
                batchSize++;
                if (batchSize % 1000 == 0) {
                    System.out.println("Processed " + batchSize + " tracks");
    ...
    public List<Track> getPopularTracks(IgniteClient client, int limit) {
        try (ResultSet<Track> resultSet = client.sql().execute(
                null, Mapper.of(Track.class), 
                "SELECT * FROM Track ORDER BY UnitPrice DESC LIMIT ?", limit)) {
            
            List<Track> tracks = new ArrayList<>(limit);
            while (resultSet.hasNext()) {
                tracks.add(resultSet.next());
            }
            return tracks;
    ...
    public List<Artist> getArtistsPage(IgniteClient client, int page, int pageSize) {
        int offset = page * pageSize;
        
        try (ResultSet<Artist> resultSet = client.sql().execute(
                null, Mapper.of(Artist.class), 
                "SELECT * FROM Artist ORDER BY ArtistId LIMIT ? OFFSET ?", pageSize, offset)) {
            
            List<Artist> artists = new ArrayList<>(pageSize);
            while (resultSet.hasNext()) {
                artists.add(resultSet.next());
            }
            return artists;
...
public class TestingStrategies {
    ...
    public class CustomerServiceTest {
        ...
        @Mock
        private RecordView<Artist> mockRecordView;
        ...
        void setUp() {
            ...
            when(mockTables.table("Artist")).thenReturn(mockTable);
            when(mockTable.recordView(Artist.class)).thenReturn(mockRecordView);
        }
        ...
        void testGetArtist_Success() {
            ...
            Artist keyMatcher = argThat(key -> artistId.equals(key.getArtistId()));
            when(mockRecordView.get(eq(null), keyMatcher)).thenReturn(expectedArtist);
            
            // When
            Artist result = artistService.getArtist(artistId);
        ...
        @Test
        void testCreateArtist_NameAlreadyExists() {
            // Given
            Artist newArtist = new Artist(null, "The Beatles");
            
            // Mock SQL query for name check
            IgniteSql mockSql = mock(IgniteSql.class);
            ResultSet<Artist> mockResultSet = mock(ResultSet.class);
    ...
    public class ArtistServiceIntegrationTest {
        ...
        void testArtistCRUD() {
            // Create
            Artist artist = new Artist(null, "Test Artist");
            Artist created = artistService.createArtist(artist);
            
            assertThat(created.getArtistId()).isNotNull();
            assertThat(created.getName()).isEqualTo("Test Artist");
            
            // Read
            Optional<Artist> retrieved = artistService.getArtist(created.getArtistId());
            assertThat(retrieved).isPresent();
            assertThat(retrieved.get().getName()).isEqualTo("Test Artist");
            
            // Update
            retrieved.get().setName("Updated Test Artist");
            Artist updated = artistService.updateArtist(created.getArtistId(), retrieved.get());
            assertThat(updated.getName()).isEqualTo("Updated Test Artist");
            
            // Delete
            artistService.deleteArtist(created.getArtistId());
            Optional<Artist> deleted = artistService.getArtist(created.getArtistId());
        ...
        private void initializeTestSchema() {
            try {
                // Create test tables
                igniteClient.catalog().createTable(Artist.class);
            } catch (Exception e) {
    ...
    public class PerformanceTest {
        ...
        void testBulkInsertPerformance() {
            // Given
            List<Artist> artists = generateTestArtists(1000);
            
            // When
            long startTime = System.currentTimeMillis();
            artistService.importArtists(artists);
            ...
            long count = artistService.countArtists();
    ...
    public static class ArtistTestDataBuilder {
        private Long artistId;
        private String name = "Test Artist";
        
        public static ArtistTestDataBuilder anArtist() {
            return new ArtistTestDataBuilder();
        }
        
        public ArtistTestDataBuilder withId(Long artistId) {
            this.artistId = artistId;
            return this;
        }
        
        public ArtistTestDataBuilder withName(String name) {
            this.name = name;
            return this;
        }
...
public class TestDataManagement {
    ...
    public static class TestDataCleanup {
        ...
        public void cleanupTestData() {
            try {
                ...
                igniteClient.sql().execute(null, "DELETE FROM Artist WHERE Name LIKE 'Test%'");
                igniteClient.sql().execute(null, "DELETE FROM Album WHERE AlbumId < 0"); // Negative IDs for test data
    ...
    public static class TestDataFactory {
        ...
        public static Artist createRandomArtist() {
            return new Artist(
                null,
                "Test Artist " + random.nextInt(1000)
            );
        }
        
        public static List<Artist> createArtists(int count) {
            return IntStream.range(0, count)
                .mapToObj(i -> createRandomArtist())
                .collect(Collectors.toList());
        }
        
        public static Artist createArtistWithGenre(String genre) {
            return new Artist(null, "Test " + genre + " Artist");
        }
    }
}
```
...
public class CacheKeyDesignPatterns {
    ...
    public static class KeyGenerator {
        ...
        public static String artistKey(Long artistId) {
            return "artist" + SEPARATOR + artistId;
        }
        
        public static String albumKey(Long albumId) {
            return "album" + SEPARATOR + albumId;
        }
        
        public static String trackKey(Long trackId) {
            return "track" + SEPARATOR + trackId;
        }
        
        // Pattern for composite keys
        public static String albumTrackKey(Long albumId, Long trackId) {
            return "album" + SEPARATOR + albumId + SEPARATOR + "track" + SEPARATOR + trackId;
        }
        
        // Pattern for filtered data
        public static String tracksByGenreKey(String genre) {
            return "tracks" + SEPARATOR + "genre" + SEPARATOR + genre.toLowerCase();
        }
    }
...
public class TTLExpirationStrategies {
    ...
    public enum CacheTTLStrategy {
        ARTIST_DATA(Duration.ofHours(24)),        // Artist information, rarely changes
        ALBUM_DATA(Duration.ofHours(12)),         // Album information
        TRACK_DATA(Duration.ofHours(6)),          // Track information
        PLAYLIST_DATA(Duration.ofHours(2)),       // User playlists, computed data
        RECOMMENDATION_DATA(Duration.ofMinutes(30)), // Music recommendations
    ...
    public static class SmartTTLCalculator {
        ...
        private Duration getBaseTTL(String dataType) {
            return switch (dataType.toLowerCase()) {
                case "artist" -> CacheTTLStrategy.ARTIST_DATA.getTtl();
                case "album" -> CacheTTLStrategy.ALBUM_DATA.getTtl();
                case "track" -> CacheTTLStrategy.TRACK_DATA.getTtl();
                case "playlist" -> CacheTTLStrategy.PLAYLIST_DATA.getTtl();
                case "recommendation" -> CacheTTLStrategy.RECOMMENDATION_DATA.getTtl();
