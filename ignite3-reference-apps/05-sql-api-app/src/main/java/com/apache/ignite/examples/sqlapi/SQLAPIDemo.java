...
public class SQLAPIDemo {
    ...
    /**
     * Demonstrates comprehensive ResultSet processing patterns and data type handling.
    ...
    private void demonstrateTransactionIntegration(IgniteClient client) {
        ...
        ResultSet<SqlRow> verification = sql.execute(null,
            "SELECT COUNT(*) as count FROM Artist WHERE Name = ?", "Demo Artist");
        
        if (verification.hasNext()) {
            long count = verification.next().longValue("count");
    ...
    private void demonstrateBatchOperations(IgniteClient client) {
        ...
        try {
            ...
            ResultSet<SqlRow> batchResult = sql.execute(null,
                "SELECT COUNT(*) as count FROM Artist WHERE Name LIKE 'Batch Artist%'");
            
            if (batchResult.hasNext()) {
                long count = batchResult.next().longValue("count");
    ...
    private void demonstrateObjectMapping(IgniteClient client) {
        ...
        // For more complex POJO mapping, you would create custom classes with @Column annotations
        // and use Mapper.of(YourClass.class) - see the documentation for detailed examples
    }
    
    /**
     * Demonstrates comprehensive error handling for SQL operations.
     * Shows how to handle different types of SQL exceptions and implement retry patterns.
     */
    private void demonstrateErrorHandling(IgniteClient client) {
        ...
        boolean success = executeWithRetry(sql, 
            "SELECT COUNT(*) as count FROM Artist", 3);
    ...
    private void demonstratePerformancePatterns(IgniteClient client) {
        ...
        Statement countryLookup = sql.statementBuilder()
            .query("SELECT COUNT(*) as count FROM Artist WHERE Country = ?")
            .build();
