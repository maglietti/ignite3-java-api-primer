...
/**
 * Demonstrates DML (Data Manipulation Language) operations using the Ignite 3
 * SQL API.
 * 
...
public class DMLOperations {
    ...
    private void demonstrateTransactionalDML(IgniteClient client) {
        ...
        try {
            ...
            try {
                // Insert artist
                sql.execute(tx1, "INSERT INTO Artist (Name, Country) VALUES (?, ?)",
                        "Transactional Artist", "TX Country");

                // Get artist ID
                ResultSet<SqlRow> artistQuery = sql.execute(tx1,
                ...
                if (artistQuery.hasNext()) {
                    ...
                    sql.execute(tx1, "INSERT INTO Album (ArtistId, Title) VALUES (?, ?)",
            ...
            try {
                // Insert valid artist
                sql.execute(tx2, "INSERT INTO Artist (Name, Country) VALUES (?, ?)",
                ...
                sql.execute(tx2, "INSERT INTO Artist (ArtistId, Name) VALUES (?, ?)",
                        1, "This should fail due to duplicate key");

                tx2.commit(); // This won't be reached if above fails
            } catch (Exception e) {
                tx2.rollback();
                logger.warn("Expected error in transaction, rolled back: {}", e.getMessage());
            }
            ...
            Transaction tx3 = client.transactions().begin();
            try {
                // Multiple related operations
                sql.execute(tx3, "INSERT INTO Artist (Name, Country) VALUES (?, ?)",
                        "Complex TX Artist", "Complex Country");

                ResultSet<SqlRow> complexArtistQuery = sql.execute(tx3,
                ...
                if (complexArtistQuery.hasNext()) {
                    ...
                    sql.execute(tx3, "INSERT INTO Album (ArtistId, Title) VALUES (?, ?)",
                            artistId, "Complex Album 1");
                    sql.execute(tx3, "INSERT INTO Album (ArtistId, Title) VALUES (?, ?)",
                            artistId, "Complex Album 2");

                    // Update artist with additional info
                    sql.execute(tx3, "UPDATE Artist SET Country = ? WHERE ArtistId = ?",
                ...
                tx3.rollback();
                logger.error("Complex transaction failed and was rolled back", e);
            }

        } catch (Exception e) {
            logger.error("Failed to perform transactional DML operations: {}", e.getMessage(), e);
        }
    }
